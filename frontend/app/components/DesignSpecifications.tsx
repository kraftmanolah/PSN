...
const DesignSpecifications: React.FC<DesignSpecificationsProps> = ({
  ...
  const handleUploadCustomDesign = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        setFileError("File size exceeds 5MB limit. Please upload a smaller file.");
        setDesignFile(null);
        if (customUploadRef.current) {
          customUploadRef.current.value = ""; // Clear the input
        }
        return;
      }
      setFileError(null);
  ...
  // Format file size for display (MB if >= 1MB, KB otherwise)
            ...
            <>
              ...
              <div className="bg-gray-100 p-3 rounded-lg w-full flex justify-between items-center">
                <div>
                  <p className="text-gray-700 font-medium">{designFile.name}</p>
                  <p className="text-xs text-green-600">
                    ✅ 100% ({formatFileSize(designFile.size)}) • Upload Successful
                  </p>
                </div>
              </div>
            </>
