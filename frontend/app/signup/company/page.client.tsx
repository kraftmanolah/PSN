...
export default function SignUpCompanyClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      !trimmedData.organization_name ||
      !trimmedData.company_address ||
      !trimmedData.account_administrator_name ||
      !trimmedData.account_administrator_role
    ) {
      setError("All company fields are required.");
      setLoading(false);
      return;
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      <main className="flex-grow flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8">
        <div className="w-full max-w-3xl bg-white rounded-lg shadow-md p-8">
          ...
          <div className="flex flex-col sm:flex-row space-x-0 sm:space-x-2 mb-6">
            <Link
              href="/signup/individual"
              className="px-3 py-1 rounded text-sm font-medium border border-gray-300 text-gray-700 hover:bg-gray-100 mb-2 sm:mb-0"
            >
              For Individuals
            </Link>
            <Link
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="col-span-1 md:col-span-2 flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-4 mt-2">
              <button
                type="submit"
                className={`px-6 py-2 font-medium rounded focus:outline-none ${
                  loading
              ...
              <Link
                href="/signin"
                className="px-6 py-2 text-center font-medium rounded border border-yellow-500 text-yellow-500 hover:bg-yellow-100 focus:outline-none"
              >
                Sign In
              </Link>
            </div>
