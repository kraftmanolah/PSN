...
//   const { data: userDataSWR, error: userError } = useSWR<UserData, Error>(
//     token ? [`${backendUrl}/api/accounts/user/`, token] : null,
//     fetcher
//   );

//   // Fetch delivery details for individuals only
//   const { data: deliveryData, error: deliveryError } = useSWR<AddressFormData, Error>(
//     token && userData?.user_type === "individual" ? [`${backendUrl}/api/accounts/delivery-details/`, token] : null,
//     fetcher
//   );
...
//     if (userDataSWR) {
//       setUserData({
//         full_name: userDataSWR.full_name || "",
//         phone_number: userDataSWR.phone_number || "",
//         email: userDataSWR.email || "",
//         user_type: userDataSWR.user_type || "individual",
//         organization_name: userDataSWR.organization_name || "",
//         company_address: userDataSWR.company_address || "",
//         delivery_details: userDataSWR.user_type === "individual" ? userDataSWR.delivery_details : null,
//       });
//     }
//     if (deliveryData && userData?.user_type === "individual") {
//       setAddressFormData(deliveryData);
//       setUserData((prev) => ({ ...prev, delivery_details: deliveryData }));
//     }
//   }, [user, token, loading, userDataSWR, deliveryData]);
...
//     try {
//       const payload = {
//         full_name: userData.user_type === "organization" ? userData.organization_name : userData.full_name,
//         organization_name: userData.user_type === "organization" ? userData.organization_name : "",
//         phone_number: userData.phone_number,
//         email: userData.email,
//       };
//       if (userData.user_type === "organization") {
//         payload.company_address = userData.company_address;
//       }
...
//       mutate(`${backendUrl}/api/accounts/user/`);
//     } catch (error) {
...
//   // Handle address update (for individuals only)
//   const handleAddressUpdate = async () => {
//     if (userData.user_type !== "individual") return;
//     try {
...
//       setShowUpdateSuccessModal(true);
//       setTimeout(() => setShowUpdateSuccessModal(false), 2000);
//       mutate(`${backendUrl}/api/accounts/delivery-details/`);
//     } catch (error) {
...
//                           {userData.user_type === "organization" ? "Organization Name" : "Full Name"}
//                         </label>
//                         <input
//                           type="text"
//                           name={userData.user_type === "organization" ? "organization_name" : "full_name"}
//                           value={userData.user_type === "organization" ? userData.organization_name : userData.full_name}
//                           onChange={handleUserDataChange}
//                           className="mt-1 block w-full border border-gray-300 rounded-md p-2"
//                           required
//                         />
//                         {errors.full_name && userData.user_type === "individual" && (
//                           <p className="text-red-500 text-sm mt-1">
//                             {Array.isArray(errors.full_name) ? errors.full_name.join(", ") : errors.full_name}
//                           </p>
//                         )}
//                         {errors.organization_name && userData.user_type === "organization" && (
//                           <p className="text-red-500 text-sm mt-1">
//                             {Array.isArray(errors.organization_name) ? errors.organization_name.join(", ") : errors.organization_name}
//                           </p>
//                         )}
//                       </div>
//                       <div>
...
//                       {userData.user_type === "organization" ? "Company Address" : "Delivery Address"}
//                     </h2>
...
//                         <p className="text-red-500 text-sm mt-1">
//                           {Array.isArray(errors.company_address) ? errors.company_address.join(", ") : errors.company_address}
//                         </p>
//                       )}
//                     </div>
...
//       {/* Address Edit Modal (for individuals only) */}
//       {showAddressModal && userData.user_type === "individual" && (
//         <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
...
// Fetcher function for UserData
const userFetcher = async ([url, token]: [string, string | null]): Promise<UserData> => {
...
// Fetcher function for AddressFormData
const addressFetcher = async ([url, token]: [string, string | null]): Promise<AddressFormData> => {
  const response = await fetch(url, {
    headers: token ? { Authorization: `Token ${token}` } : {},
  });
  if (!response.ok) throw new Error(`Fetch failed with status: ${response.status}`);
